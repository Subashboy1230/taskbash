...
// shadcn/ui Sheet — slide-in panel built on Radix Dialog.
// Used for the right-side DetailPanel in Phase 2.
...
>(({ className, ...props }, ref) => (
  // Transparent overlay: still catches click-outside to close the sheet,
  // but does not dim or blur the page behind it. User wants the task list
  // to stay fully visible while the detail panel is open.
...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-background border-border p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300',
  {
    variants: {
      side: {
